...
if (empty($_SESSION['username']) && empty($_SESSION['password'])){
	...
	if ($_GET['mod'] == 'ubah_password'){
		...
		//if ($_SESSION['level'] == 'dos'){
			$sql_user = $db->database_prepare("SELECT password FROM master_data WHERE id_karyawan = ?")->execute($_SESSION["useri"]);
		//}
		...
		if ($pass_lama != $data_user['password']){
			...
			if ($pass_baru != $pass_baru2){
				...
				//if ($_SESSION['level'] == 'dos'){
					$db->database_prepare("UPDATE master_data SET password = ? WHERE id_karyawan = ?")->execute($pass_baru,$_SESSION["useri"]);
				//}
				//elseif ($_SESSION['level'] == 1){
